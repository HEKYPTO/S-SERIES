...
fn run_trial(words: &[String]) -> (f64, usize, usize) {
    let start = Instant::now();
    let rainbow_table = create_table(words);
    let creation_time = start.elapsed().as_secs_f64();
    let table_size = rainbow_table.len();
    let table_memory_size = rainbow_table.iter()
        .map(|(k, v)| k.len() + v.len())
        .sum();
    (creation_time, table_size, table_memory_size)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut table_memory_sizes = Vec::new();

    for i in 0..num_trials {
        println!("Running trial {}/{}", i + 1, num_trials);
        let (creation_time, table_size, table_memory_size) = run_trial(&words);
        creation_times.push(creation_time);
        table_sizes.push(table_size);
        table_memory_sizes.push(table_memory_size);
    }
    ...
    let avg_table_memory_size: f64 = table_memory_sizes.iter().sum::<usize>() as f64 / num_trials as f64;
    ...
    println!("Average table memory size: {:.2} MB", avg_table_memory_size / (1024.0 * 1024.0));
